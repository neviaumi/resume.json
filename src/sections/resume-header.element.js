      ...
      <header class="${clsx('tw-mb-2 tw-flex tw-flex-wrap-reverse tw-justify-between tw-px-2 tw-pb-1 tw-pt-2')}">
        <div class="${clsx('tw-flex tw-flex-col tw-justify-between print:tw-break-inside-avoid')}">
          <div>
            ...
            <p class="${clsx('tw-text-3xl tw-font-normal tw-text-placeholder')}">${resumeBasic.label}</p>
          </div>
          <div>
              <a href="mailto:${resumeBasic.email}?subject=${encodeURIComponent(
                'Job opportunity',
              )}" class="${clsx('tw-flex tw-content-center tw-items-center tw-gap-2 tw-text-base tw-font-semibold tw-no-underline')}">
                <img class="${clsx('tw-h-3')}" src="${envelopeIcon}" alt="email-icon">
                ${resumeBasic.email}
              </a>
              <a href="resume.pdf" target="_blank" class="${clsx('tw-flex tw-gap-2 tw-text-base tw-font-semibold  tw-no-underline print:tw-hidden')}">
                <img class="${clsx('tw-h-3')}" src="${printerIcon}" alt="printer-icon">
                <span>PDF version</span>
              </a>
            <a href="${resumeBasic.website}" target="_blank" class="${clsx('tw-hidden tw-content-center tw-items-center tw-gap-2 tw-text-base tw-font-semibold tw-no-underline print:tw-flex')}">
